...
actually higher with quarantine** (312 infected nodes on average) than
without it (266), a difference of **-46 nodes**. This contrasts with the
previous high-β case, which was comparable in the long run.

[**\~\~\~ PLEASE CHECK ALL THE UPDATES ARE CLEAR... I think the second
plot had the wrong beta value (wasn't at 0.01 so the plots have
updated). Hopefully I haven't actually ruined it though, but the second
plot now has much lower growth... I am not sure it looks correct to be
honest...**]{.underline}

[**\~\~\~ Careful with copying the AI interepretations too! It's very
detailed and quite hard to interpret. I think simple and shorter is
better (even if you get AI to summarise their own output it is easier
for us to check ourselves!)**]{.underline}

This reversal suggests that under low-transmission conditions, the
quarantine intervention—at least in the way it is currently implemented—
is ineffective.
...
Using the code from the slides, we plot the 2 trends to compare the
outputs over time initially.

Let's visualize the evolution together with the realization with no
vaccination we got earlier.

We see that Random vaccination is better than no vaccination. But
targeted is much more effective and essentially flattened the curve.
Growth never got exponential.

```{r}
ggplot() + 
  geom_line(data=realization,aes(x=t,y=ninf,col="No Vaccination")) +
...
[**\~\~\~\~ TO REVIEW... There is too much text here and seems to be all
AI output... really need to simplify. But I can come
back.**]{.underline} Blas was clear to avoid having obvious AI output
and the text and code is very clear. The code in the workshop is much
more simple, so it's very clear and many of the variables are not
defined.

In this section, we evaluated the effectiveness of two immunization
strategies by comparing their impact on the spread of information in the
network. In both scenarios, 5% of the nodes were made “immune” (i.e.,
they did not participate in spreading the information), either through
random selection or by targeting the most central nodes based on
PageRank. We then simulated the SIR model using a 1% random selection of
the remaining nodes as initial spreaders and observed the resulting
infection trends.

**According to our previous two plots**, surprisingly, t**he random
immunization strategy outperformed the targeted PageRank-based
strategy**. The random strategy resulted in near-zero propagation, with
infections rarely exceeding a count of one or two individuals throughout
the simulation. In contrast, the PageRank-targeted strategy led to a
substantial outbreak, peaking at over 40 simultaneous infections before
declining. This contradicts the common assumption that removing the most
central nodes is always the most effective containment strategy.

**Several factors may explain this outcome**. First, the PageRank nodes
likely formed a densely interconnected core, and their removal may have
left the rest of the network still sufficiently connected for diffusion
to occur. Meanwhile, the randomly selected nodes were more evenly
distributed across the network, inadvertently severing critical bridges
between different communities and halting the spread. Additionally,
while the targeted strategy removed significantly more edges (1,867 vs
488), this did not translate into a better containment
outcome—suggesting that where nodes are removed may be more important
than how many edges are lost.

These findings highlight the importance of considering the spatial
distribution and structural role of nodes in designing immunization
strategies. In this realization, the random strategy disrupted the
network more effectively, despite its apparent lack of sophistication.
...
of 20 times instead of comparing just a pair of random results to obtain
...
ggplot(results_5c, aes(x = strategy, y = total_infected, fill = strategy)) +
  geom_boxplot() +
  geom_jitter(width = 0.1, size = 2, alpha = 0.7) +  # Each dot is one simulation
  labs(title = "Total Infected (20 runs): Random vs PageRank Kind People",
       y = "Total number of infected nodes") +
  theme_minimal()

ggplot(results_5c, aes(x = strategy, y = peak_time, fill = strategy)) +
  geom_boxplot() +
  geom_jitter(width = 0.1, size = 2, alpha = 0.7) +  # Each dot is one simulation
  labs(title = "Peak Time (20 runs): Random vs PageRank Kind People",
       y = "Time step of peak") +
  theme_minimal()
```

Based on the 20 repeated simulations conducted in Step 5c, we observe a
reversal of the conclusion reached in the single-run analysis (Step 5b).
While the earlier single realization suggested that the **random
immunization strategy** was more effective—yielding significantly fewer
infections—the repeated simulations tell a more nuanced story.

We observed some interesting findings:

-   The **PageRank-based strategy** resulted in a lower average number
    of infected nodes (mean = **774.45**, sd = 17.58), compared to the
    **random strategy** (mean = **842.75**, sd = 14.42).

-   Moreover, the **peak time** of infections under the PageRank
    strategy was **delayed** (mean = **15.75**) compared to the random
    case (mean = **9.4**), suggesting that the outbreak unfolded more
    slowly when high-centrality nodes were immunized.

-   Both differences—total infections and peak delay—are statistically
    meaningful, as the **standard deviations are small**, and the
    distributions are clearly separated in the boxplots.

This result aligns with theoretical expectations: **targeting the most
central nodes for immunization is generally more effective** at limiting
epidemic spread, since those nodes are key hubs for transmission. The
earlier contradictory result from Step 5b can be attributed to
**stochastic variability** in a single realization, especially under low
β values (we used β = 0.05). In that single run, it's possible the
randomly removed nodes happened to disrupt crucial network bridges or
coincidentally blocked early transmission routes, creating an atypical
suppression of spread.

However, after running 20 simulations for each strategy, the **aggregate
trend confirms that targeted intervention performs better on average**.
The PageRank strategy not only **reduces the overall size of the
outbreak**, but also **delays its peak**, which is an important public
health benefit—offering more time to react or intervene.

These findings highlight the **importance of repeated simulations** to
capture reliable statistical patterns and avoid misleading conclusions
based on single runs. While randomness may occasionally outperform
structured strategies by chance, **targeted immunization based on
network centrality is more consistently effective** in containing the
spread of information or contagion across a social network.
...
the importance of central nodes when consdiering the spread of rumours
in the univeristy email system. These **central nodes could be
considered the super-spreaders** at risk of creating a more rapid
epidemic.
