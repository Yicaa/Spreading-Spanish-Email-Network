...
YIJIA -\> The findings depend on how infectious the model is. I
accidentally had beta = 10% and the vaccination did help ,But then I had
it set to 1% and it doesn't help. So it seems like it depends which beta
we have...the question says to use the beta value selected in Q3 but
maybe we also show it at some other different levels below with other
box plots just to show it helps at different levels (and I think we
don't need to run 50 times each, it's too slow. 20 is plenty!)

-   This is really good though - the box plots are especially nice

We surprisingly observed that after running 50 simulations for each
...
```{r}
# We run the simulation 50 times again, as previously we only obtained the final infected number, but we didn't register the infected number at each stage
set.seed(123)
runs_no_q <- map(1:50, function(i) {
  sim <- sim_sir(graph, beta, mu, seeds)
  sim$run <- i
  sim
})

runs_q <- map(1:50, function(i) {
...
ggplot(cumulative_all, aes(x = t, y = mean_inf, color = strategy, fill = strategy)) +
  ...
  labs(title = "Infection Spread Over Time",
       subtitle = "With vs Without Quarantine (50 simulations each)",
       x = "Time step (t)",
