...
\
\*\*\* Q FOR YIJIA - WHERE DOES IT SAY UNDIRECTED NETWORKS ONLY?

-   I think it makes sense but can't find anywhere to say it. I also get
    a different output for the threshold if I use there 2 options.. we
    should confirm which we use and why.

    -   graph2 \<- graph_from_data_frame(links, directed = FALSE,
        vertices = nodes)

    -   graph \<- as_undirected(graph, mode = "collapse")

I tested all the methods of making undirected and collapse is the most
extreme (it does make sense though). But with collapse, it drops the
number of edges from 10,903 to 5,451.

If we treat it as undirected, we should understand and things like
average degree have changed.
...
# testing other graph direction values
graph2 <- graph_from_data_frame(links, directed = FALSE, vertices = nodes)
graph3 <- graph <- as_undirected(graph, mode = "each")
graph4 <- graph <- as_undirected(graph, mode = "mutual")

# check edge counts
igraph::ecount(graph2)
igraph::ecount(graph_undirected)

# Apply the formula: βc = μ * <k> / <k^2>
mu * mean(degree(graph_undirected)) / (mean(degree(graph_undirected)^2))
mu * mean(degree(graph)) / (mean(degree(graph)^2))
mu * mean(degree(graph2)) / (mean(degree(graph2)^2))
mu * mean(degree(graph3)) / (mean(degree(graph3)^2))
mu * mean(degree(graph4)) / (mean(degree(graph4)^2))

# the threshold
beta_c <- mu * mean(degree(graph_undirected)) / (mean(degree(graph_undirected)^2))
...
TO DO: we need to plot the growth over time as well. This is then
compared to in part 3.

```{r}
# plot the outputs over time
results <- map_dfr(seq(0,0.2,0.01), # beta range
        \(beta){ seeds <- sample(1:vcount(graph2),vcount(graph2)*0.01)
        realization <- sim_sir(graph2,beta,mu,seeds) # make a realization for each beta
...
Then we can make a more detailed view because the rise is so sharp, to
try view the change very close to our critical threshold and how it
changes. We plot the 10 values either side of our critical threshold.
Noting these are very minuscule values so noise exists between values.

```{r}
# plot detailed outputs very close to threshold
results <- map_dfr(seq(0,beta_c*2,beta_c/5), # beta range
        \(beta){ seeds <- sample(1:vcount(graph2),vcount(graph2)*0.01)
        realization <- sim_sir(graph2,beta,mu,seeds) # make a realization for each beta
        data.frame(beta,ninf=nrow(realization)) # create dataframe row
        })
